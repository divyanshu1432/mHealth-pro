...
const [filterstyle, setfilterstyle] = useState({display: 'flex', flexWrap: 'wrap' , display:'none'
})
...
function sel(e){
  setmystyle(setcardstyle),
  setfilterstyle(setfiltercardstyle)
  ...
  useEffect(() => {
    ...
    setmystyle(agcardstyle),
    setfilterstyle(agsetcardstyle)
    setFilteredSubEvent([]);
  }, [eventId]);
  ...
  const renderSubEventList = () => {
    const data =
      selectedFilter !== 'all'
        ...
        useEffect(() => {
          let unique = [...new Map(data.map((item) => [item['eventType'] , item])).values()];
      ...
      >
        ...
        >
          <TriStateToggle
            ...
            handleChange={(value) =>{ setSelectedFilter(value) }}
          />
        ...
        <div>
        <select onChange={sel} style={{height:'30px' , fontSize:'12px' , baackground:'transparent'}}> 
